                ...
                <form action="{{ route('murid.update', $murid->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group my-2">
                        ...
                        <input type="text" name="nama_murid" id="nama_murid"
                            class="form-control @error('nama_murid') is-invalid @enderror" value="{{ $murid->nama_murid }}"
                            autofocus>
                        @error('nama_murid')
                        ...
                        <select name="bahagian_id" id="bahagian_id"
                            ...
                            @foreach ($bahagians as $bahagian)
                                <option value="{{ $bahagian->id }}"
                                    {{ $murid->bahagian_id == $bahagian->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group my-2">
                        ...
                        <input type="text" name="alamat" id="alamat"
                            class="form-control @error('alamat') is-invalid @enderror" value="{{ $murid->alamat }}">
                        @error('alamat')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group my-2">
                        ...
                        <input type="number" name="id_murid" id="id_murid"
                            class="form-control @error('id_murid') is-invalid @enderror" value="{{ $murid->id_murid }}"
                            readonly>
                        @error('id_murid')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
